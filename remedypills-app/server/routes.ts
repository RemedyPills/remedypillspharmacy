 ...
 */
function verifyFacebookSignedRequest(signedRequest: string, appSecret: string): { data: { object_id?: string; user_id?: string; data_deletion_id?: string } } | null {
  try {
...
export async function registerRoutes(
  ...
  app.get("/api/admin/health-logs", requireAdmin, async (req, res) => {
    ...
    if (userId) {
      ...
      const allUsers = await storage.getAllUsers();
      const patients = allUsers.filter((u: any) => u && u.role === "patient");
      const allLogs: any[] = [];
